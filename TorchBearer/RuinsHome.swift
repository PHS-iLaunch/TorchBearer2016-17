...
class RuinsHome: SKScene {
    ...
    override func didMove(to view: SKView) {
        ...
        let nextLevel = SKSpriteNode(imageNamed: "Arrow")
        ...
        let previousLevel = SKSpriteNode(imageNamed: "Arrow")
        previousLevel.setScale(0.01)
        previousLevel.zRotation = CGFloat(M_PI)
        previousLevel.position = CGPoint(x:self.frame.width/2,y:self.frame.height-nextLevel.size.height/2)
        previousLevel.zPosition = 6
        previousLevel.pulsate()
        self.addChild(previousLevel)
