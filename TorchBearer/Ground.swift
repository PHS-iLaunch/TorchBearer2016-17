...
class Ground:SKSpriteNode{
    ...
    init(scene:SKScene){
        ...
        self.size = CGSize(width:scene.frame.width,height:scene.frame.height/GameConstants.floorHeightConstant)
        self.zPosition = 3
        self.name = "ground"
        self.position = CGPoint(x:scene.frame.width/2,y:self.size.height/2)
        setPhysics()
        makeAction(true)
    ...
    init(scene:SKScene, xPos:CGFloat){
        ...
        self.zPosition = 3
        self.name = "ground"
        self.position = CGPoint(x:xPos,y:self.size.height/2)
        setPhysics()
        makeAction(false)
    ...
    func setPhysics(){
        ...
        self.physicsBody?.categoryBitMask = PhysicsMasks.Ground
